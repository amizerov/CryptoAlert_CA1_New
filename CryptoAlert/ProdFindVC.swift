...
class ProdFindVC: UIViewController,
	...
	func updateSearchResults() {
		let txtSearch = searchBar.text!.uppercased()
		let txtFilter = searchBar.scopeButtonTitles![searchBar.selectedScopeButtonIndex]
		if !txtSearch.isEmpty {
			if !(txtFilter == "All") {
				arFiltr = arProds.filter {
					$0.symbol.uppercased().contains(txtSearch)
					&& $0.symbol.contains(txtFilter)
				}
			}
			else {
				arFiltr = arProds.filter {
			...
			if !(txtFilter == "All") {
				arFiltr = arProds.filter {
					$0.symbol.uppercased().contains(txtFilter)
				}
			}
			else {
				arFiltr = arProds
